...
soap_wsdd_Probe(struct soap *soap, soap_wsdd_mode mode, soap_wsdd_to to, const char *endpoint, const char *MessageID, const char *ReplyTo, const char *Types, const char *Scopes, const char *MatchBy)
{
    ...
    if (Scopes)
    {
        ...
        qDebug()<<"scopes.__item : "<<scopes.__item<<" scopes.MatchBy : "<<scopes.MatchBy;
    ...
    else
    {
        ...
        {
            qDebug()<<"soap->header->wsdd__AppSequence : "<<soap->header->wsdd__AppSequence;
        ...
        qDebug()<<"kamrul-4 InstanceId : "<<InstanceId<<" SequenceId : "<<SequenceId<<" MessageNumber : "<<MessageNumber;
...
soap_wsdd_listen(struct soap *soap, int timeout)
{
    ...
    for (;;)
    {
        ...
        int capture = soap_wsdd_serve_request(soap);
        if (capture)
        {
